...
*/
bool Character::operator==(const Character& player) const {
    return (name_ == player.name_ && race_ == player.race_ && level_ == player.level_ && enemy_ == player.enemy_);
}
...
/**
  ...
  @return     : Returns true if the right hand side character is NOT "equal" (!=), false        
                ...
                NOTE: By this definition, one or more of the aforementioned subset of the                       character's attributes only must be different for two characters to be 
                deemed "NOT equal".
*/
bool Character::operator!=(const Character& player_) const {
    return !(*this == player_);
}
...
std::string Character::display() {
    std::string enemy_status = (enemy_ ? "an enemy" : "not an enemy");
    return "[" + name_ + "] is a Level " + std::to_string(level_) + " " + getRace() + ". \nVitality: " +
           std::to_string(vitality_) + "\nMax Armor: " + std::to_string(armor_) + "\nThey are " + enemy_status + ".\n";
}
